				...
				<div class="entry" <?php if ( has_post_thumbnail() && $featured_images == '1' && !is_single()  ) { echo 'style="min-height: 115px;" '; }?>>
					<?php 
						...
						the_excerpt();
							...
							the_content(__('Leer ma´s...', 'response'));
						}
					 ?>
				</div><!--end entry-->
...
function response_post_bar_content() { 
	...
	<div id="comments">
	<?php if (($hidden[$themeslug.'_hide_comments']) != '0'):?><?php comments_popup_link( __('Sin comentarios', 'response' ), __('1 Comentarios', 'response' ), __('% Comentarios' , 'response' )); //need a filer here ?>.<?php endif;?>
	</div>
	<?php
}
...
function response_post_byline_content() {
	...
	<div class="meta">
		<?php if (($hidden[$themeslug.'_hide_date']) != '0'):?> <?php printf( __( 'Publicado el', 'response' )); ?> <a href="<?php the_permalink() ?>"><?php echo get_the_date(); ?></a>,<?php endif;?>
		<?php if (($hidden[$themeslug.'_hide_author']) != '0'):?><?php printf( __( 'por', 'response' )); ?> <?php the_author_posts_link(); ?> <?php endif;?> 
		<?php if (($hidden[$themeslug.'_hide_categories']) != '0'):?><?php printf( __( 'en', 'response' )); ?> <?php the_category(', ') ?>.<?php endif;?>
...
function response_post_tags_content() {
	...
	<div class="tags">
			<?php the_tags( __('Etiquetas: ', 'response'), ', ', '<br />'); ?>
